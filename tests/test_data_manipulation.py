...
def test_fun(num):
    return "1-7-3-4-6-7-3-2-1-4-7-6-Charlie-3-2-7-8-9-7-7-7-6-4-3-Tango-7-"+ \
...
class TestLoadSave(unittest.TestCase):
    ...
    def test_saving(self):
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        loadSave(test_fun, "test_loadSave", force_save=False, args=(5,), path=self.path_data)

        self.assertTrue(os.path.exists(file_path))
    
    def test_loading(self):
        data = test_fun(5)
        data_loaded = loadSave(test_fun, "test_loadSave", force_save=False, args=(5,), path=self.path_data)
